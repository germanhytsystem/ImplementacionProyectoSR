...
async def get_movie(id: str, q: int = 0, p: int = 5):
    ...
    ids_filtradas = schema_ratings(
        db_client.Movilends.ratings.find({"movieId": id}))[q:q+p]

    if not ids_filtradas:
        raise HTTPException(
            status_code=404, detail="No se encontraron movieId ")
...
async def get_user(id: str, q: int = 0, p: int = 5):
    ...
    ids_filtradas = schema_ratings(
        db_client.Movilends.ratings.find({"userId": id}))[q:q+p]

    if not ids_filtradas:
        raise HTTPException(
            status_code=404, detail="No se encontraron userId ")
...
async def Rating(rati: Rating):
    ...
    nuevo_doc = schema_rating(
        db_client.Movilends.ratings.find_one({"_id": nuevo_id}))
